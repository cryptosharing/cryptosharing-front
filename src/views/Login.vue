  ...
  <div class="container">
    <div class="home">
      ...
      <div class="content" v-if="tab == 'rent'">
        <div class="nologin" v-if="!isLogin">You don't have any NFTs to rent</div>
        <div v-if="isLogin">
          <div class="title">Explore Rent Collections</div>
          <div class="main-content" v-if="!rentDetails && !_rentLoading">
            <div v-for="(item,index) in rentGameList" :key="index" class="item" @click="showRentGameDeail(item._address,index)">
          ...
          <div class="detail_bag" v-if="rentDetails && !_rentLoading">
            <div class="detail_total">
              {{rentDetails.length}} results
            </div>
          <div class="main-content">
            <div v-for="(id,index) in rentDetails" :key="index" class="min-item">
              <img :src="lendImg[index]" />
              <div class="name">{{id}}</div>
              <div class="desc">
                <a-button type="primary" @click="rentItem(id,index)">Rent</a-button>
              </div>
            </div>
          </div>
          </div>
      ...
      <div class="content" v-if="tab == 'lend'">
        <div class="nologin" v-if="!isLogin">You don't have any NFTs to lend</div>
        <div v-if="isLogin">
          <div class="title">Explore Lend Collections</div>
          <div class="main-content" v-if="!lendDetails && !_lendLoading">
            <div v-for="(item,index) in lendGameList" :key="index" class="item" @click="showLendGameDeail(item,index)">
              <img :src="lendGameImg[index]" />
              <div class="name">{{item._name}}</div>
              <div class="name-by">
                <span style="color: #2081e2;">{{item._symbol}}</span>
          ...
          <div class="detail_bag" v-if="lendDetails && !_lendLoading">
            <div class="detail_total">
              {{lendDetails.length}} results
            </div>
          <div class="main-content">
            <div v-for="(id,index) in lendDetails" :key="index" class="min-item">
              <img :src="lendImg[index]" />
              <div class="name">{{id}}</div>
              <div class="desc">
                <a-button type="primary" @click="lendItem(id,index)">Lend</a-button>
              </div>
            </div>
          </div>
          </div>
    ...
    <div>
      ...
      >
        <div class="content" v-if="isLogin">
          <div class="title">
            <div class="online"><img src="../assets/online.png" /></div>
            <a-collapse
              style="width: 130px;"
              class="expand"
              v-model="activeKey"
    ...
    >
      <a-form :model="rentFormState">
        <a-form-item label="how many day you want rent" name="rentDay">
          <a-input v-model:value="rentFormState.rentDay" type="number" placeholder="fill nft price" />
        </a-form-item>
      </a-form>
    </a-modal>
    ...
    >
      <a-form :model="lendFormState">
        ...
        <a-form-item label="lend how many nft a day" name="lendMoney">
          <a-input v-model:value="lendFormState.lendMoney" placeholder="fill nft price" />
        </a-form-item>
      </a-form>
    </a-modal>
  </div>
...
export default {
  ...
  setup() {
    ...
    const confirmLend = async () =>{
      ...
      if(lendTime<= now) return message.error('lend date must be greater than now!');
      if(!state.lendFormState.lendMoney) return message.error('price require!');
      ...
      console.log(state.lendAddressIndex);

      const axsAddress =  state.axsRentAddress[state.lendAddressIndex]; // "0x20366D9EEDFFFA1aF4f7Cf0394D77c772258a8D0";
      //const axsAddress =  "0x20366D9EEDFFFA1aF4f7Cf0394D77c772258a8D0";
      console.log(axsAddress);
      console.log(providerSign);
      ...
      //const axsNFTAddress =  "0x2f359E67aFe0b0A11D7715bDDe889211C518828f"
      const axsNFTContract = new ethers.Contract(axsNFTAddress, axsLendAbi, providerSign);

      console.log(axsAddress);
      console.log(state.tokenId);
      state.confirmLendLoading = true;
      axsNFTContract.approve(axsAddress, state.tokenId).then((result) => {
        console.log(result);
        setTimeout(() => {
          axsContract.lendNFT(state.tokenId,Math.round(lendTime/1000),lendMoney).then(res=>{
            console.log(res);
    ...
    const confirmRent = async () =>{
      if(!state.rentFormState.rentDay ||state.rentFormState.rentDay<=0) return message.error('rent day error!');
      ...
      const axsContract = new ethers.Contract(state.axsRentAddress[state.rentAddressIndex], axsAbi, providerSign);
      ...
      USDTContract.approve(state.axsRentAddress[state.rentAddressIndex],price).then((result) => {
        console.log(result);
        state.confirmRentLoading = true;
        setTimeout(() => {
          const time  = nowtime + rentDay * 3600 * 24;
          console.log(nowtime);
          console.log(time);
          console.log(state.tokenId);
          axsContract.rentNFT(state.tokenId, time).then(res=>{
            console.log(res);
